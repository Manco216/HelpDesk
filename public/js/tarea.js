window.Tarea = (function () {
  ...
  function openCreateForm() {
    ...
    body.innerHTML = [
      ...
      '  <div class="form-row"><label>Nombre</label><input type="text" class="task-nombre input-min" placeholder="Nombre de la tarea"></div>',
    ...
    var nombreEl = body.querySelector(".task-nombre");
  ...
  function openEditForm(id) {
    ...
    (function () {
      ...
      if (h) h.textContent = "Editar tarea";
      if (tag) tag.textContent = "Editar tarea";
    })();
    body.innerHTML = [
      '<div class="admin-form">',
      '  <div class="form-row"><label>Nombre</label><input type="text" class="task-nombre input-min" placeholder="Nombre de la tarea"></div>',
    ...
    fetch(base() + "/api/admin/tasks")
      ...
      .then(function (rows) {
        ...
        if (match && nombreEl) nombreEl.value = String(match.nombre_tarea || "");
      })
      .catch(function () {});
    if (btnSave) {
      btnSave.addEventListener("click", function () {
        ...
        if (!nombre) { alert("Nombre es obligatorio"); return; }
        var token = getCsrfToken();
        fetch(base() + "/api/admin/tasks/" + encodeURIComponent(String(id)), {
          method: "PUT",
          credentials: "same-origin",
          headers: Object.assign({ "Content-Type": "application/json" }, token ? { "X-CSRF-TOKEN": token } : {}),
          body: JSON.stringify({ nombre: nombre })
