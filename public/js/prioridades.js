window.Prioridad = (function () {
  ...
  function openCreateForm() {
    ...
    body.innerHTML = [
      ...
      '  <div class="form-row"><label>Nombre</label><input type="text" class="prioridad-nombre input-min" placeholder="Nombre de la prioridad"></div>',
    ...
    var nombreEl = body.querySelector(".prioridad-nombre");
    ...
    if (btnSave) {
      btnSave.addEventListener("click", function () {
        ...
        fetch(base() + "/api/admin/priorities", {
          method: "POST",
  ...
  function openEditForm(id) {
    ...
    (function () {
      ...
      if (h) h.textContent = "Editar prioridad";
      if (tag) tag.textContent = "Editar prioridad";
    })();
    body.innerHTML = [
      '<div class="admin-form">',
      '  <div class="form-row"><label>Nombre</label><input type="text" class="prioridad-nombre input-min" placeholder="Nombre de la prioridad"></div>',
    ...
    fetch(base() + "/api/admin/priorities")
      ...
      .then(function (rows) {
        ...
        if (match && nombreEl) nombreEl.value = String(match.nombre_prioridad || "");
      })
      .catch(function () {});
    if (btnSave) {
      btnSave.addEventListener("click", function () {
        ...
        if (!nombre) { alert("Nombre es obligatorio"); return; }
        var token = getCsrfToken();
        fetch(base() + "/api/admin/priorities/" + encodeURIComponent(String(id)), {
          method: "PUT",
          credentials: "same-origin",
          headers: Object.assign({ "Content-Type": "application/json" }, token ? { "X-CSRF-TOKEN": token } : {}),
          body: JSON.stringify({ nombre: nombre })
