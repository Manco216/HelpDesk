window.Departamento = (function () {
  ...
  function openCreateForm() {
    ...
    body.innerHTML = [
      ...
      '  <div class="form-row"><label>Nombre</label><input type="text" class="dept-nombre input-min" placeholder="Nombre del departamento"></div>',
    ...
    var nombreEl = body.querySelector(".dept-nombre");
    ...
    if (btnSave) {
      btnSave.addEventListener("click", function () {
        ...
        fetch(base() + "/api/admin/departments", {
          method: "POST",
  ...
  function openEditForm(id) {
    ...
    (function () {
      ...
      if (h) h.textContent = "Editar departamento";
      if (tag) tag.textContent = "Editar departamento";
    })();
    body.innerHTML = [
      '<div class="admin-form">',
      '  <div class="form-row"><label>Nombre</label><input type="text" class="dept-nombre input-min" placeholder="Nombre del departamento"></div>',
    ...
    fetch(base() + "/api/admin/departments")
      ...
      .then(function (rows) {
        ...
        if (match && nombreEl) nombreEl.value = String(match.nombre_departamento || "");
      })
      .catch(function () {});
    if (btnSave) {
      btnSave.addEventListener("click", function () {
        ...
        if (!nombre) { alert("Nombre es obligatorio"); return; }
        var token = getCsrfToken();
        fetch(base() + "/api/admin/departments/" + encodeURIComponent(String(id)), {
          method: "PUT",
          credentials: "same-origin",
          headers: Object.assign({ "Content-Type": "application/json" }, token ? { "X-CSRF-TOKEN": token } : {}),
          body: JSON.stringify({ nombre: nombre })
